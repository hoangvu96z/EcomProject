...
class SimprovidersControllerSimprovider extends JController
{
    function __construct()
    {
        ...
        if($tasks == "unpublish")
        {
            if($cid == array()){
                ...
                $this->setRedirect( 'index.php?option=com_simprovider',$msg);
            }else{
    ...
    function edit()
    {
        JRequest::setVar( 'view', 'simprovider' );
    ...
    function save()
    {
        $id = JRequest::getVar('provider_id',0);
        $cid = JRequest::getVar('cid',array(),'','array');
        $model = $this->getModel('simprovider');
        if($cid != array()){
            //is Publish or Unpublish
            for($i=0,$n=count($cid);$i<$n;$i++){
                JRequest::setVar( 'provider_id',$cid[$i]);
                if ($model->store($post)) {
                    $msg = JText::_( 'Success!' );
                } else {
                    $msg = JText::_( 'Error' );
        ...
        $link = 'index.php?option=com_simprovider';
