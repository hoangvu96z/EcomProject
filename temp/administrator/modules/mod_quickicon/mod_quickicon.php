...
if (!defined( '_JOS_QUICKICON_MODULE' ))
{
	...
	<div id="cpanel">
        ...
        if ( $user->get('gid') > 24 ) {
			...
			quickiconButton( $link, '2005_8_sim_0.jpg', JText::_( 'SIM888-Đặt hàng SIM' ) );
		}
        if ( $user->get('gid') > 24 ) {
			$link = 'index.php?option=com_sim';
			quickiconButton( $link, 'ThailandSIM.jpg', JText::_( 'SIM888-Dsách sim' ) );
		}
        if ( $user->get('gid') > 24 ) {
			$link = 'index.php?option=com_sim&task=edit&controller=sim';
			quickiconButton( $link, 'newsim.jpg', JText::_( 'SIM888-Thêm mới sim' ) );
		}
		?>
	</div>
	<?php
